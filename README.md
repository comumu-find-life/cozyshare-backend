...
💡 GitActions 을 사용하면 Github 에서 코드 버전 관리가 이루어지고 코드가 푸시될 때마다 배포가 자동화 된다는 장점이 있습니다.
   YAML 파일로 간단히 CI/CD 파이프 라인을 설정할 수 있습니다. Jenkis 를 통해 설정도 가능하지만, 
   복잡한 플러그인 설치 과정이 필요하고 별도의 관리가 필요해 개인 프로젝트에서 효율이 떨어질 수 있다 생각했습니다.
```
...
```
💡 지도를 이용한 집 게시물 조회, 도시 이름을 이용한 집 게시물 조회 등 서로 다른 API지만 동일한 데이터를 요구합니다. 
  때문에 해당 데이터들을 Redis를 통해 성능을 개선했고, 100개 데이터 기준으로 291 ms -> 180 ms 로 단축했습니다.
  이때 데이터의 일관성이 깨질 수 있음을 유의하며 Redis 를 적용했습니다. 
  또한 Redis 는 master-slave 구조로 배포되어 읽기 성능을 향상시키고, 데이터 부하를 분산시켰습니다.
```
...
```
💡 금융 시스템에서 정확성과 일관성이 가장 중요합니다. 때문에 결제 처리, 잔액 업데이트같은 기능은 동시성이 보장되지 않으면 
   데이터 정합성 오류나 중복 처리 문제가 발생할 수 있습니다.
